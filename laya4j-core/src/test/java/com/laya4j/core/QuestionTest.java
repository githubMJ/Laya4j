...
class QuestionTest {
    ...
    @Test
    void layaConfigBuilder() {
        LayaConfig c = LayaConfig.builder()
                .maxLen(512).headMaxLen(128)
                .temperature(1.2, 0.9, 1.0)
                .build();
        assertEquals(512, c.maxLen());
        assertEquals(128, c.headMaxLen());
        assertArrayEquals(new double[]{1.2, 0.9, 1.0}, c.temperature());
        assertEquals(1.2, c.temperatureFor(DecisionType.CHOICE));
        assertEquals(0.9, c.temperatureFor(DecisionType.SCORE));
        assertEquals(1.0, c.temperatureFor(DecisionType.NOUL));
    }

    @Test
    void layaConfigValidation() {
        assertThrows(LayaException.class, () -> LayaConfig.builder().maxLen(0).build());
        assertThrows(LayaException.class, () -> LayaConfig.builder().headMaxLen(0).build());
        assertThrows(LayaException.class, () ->
                LayaConfig.builder().maxLen(100).headMaxLen(200).build());
        assertThrows(LayaException.class, () ->
                LayaConfig.builder().temperature(1.0, -1.0, 1.0).build());
    }

    @Test
    void layaConfigFromRlAgentConfigFallsBackWhenMissing() {
        // 文件不存在时优雅降级为 defaults(),不抛异常
        // 注意:LayaConfig 含数组字段,record 生成的 equals() 对数组是引用比较,
        // 因此逐字段比较而非直接 assertEquals(record, record)
        LayaConfig c = LayaConfig.fromRlAgentConfig(java.nio.file.Path.of("/no/such/file.json"));
        LayaConfig d = LayaConfig.defaults();
        assertEquals(d.maxLen(), c.maxLen());
        assertEquals(d.headMaxLen(), c.headMaxLen());
        assertArrayEquals(d.temperature(), c.temperature());
    }

    @Test
    void layaConfigFromRlAgentConfigParsesRealFile(@org.junit.jupiter.api.io.TempDir java.nio.file.Path tmp)
            throws Exception {
        java.nio.file.Path f = tmp.resolve("rl_agent_config.json");
        java.nio.file.Files.writeString(f,
                "{\"max_len\": 2048, \"head_max_len\": 512, \"temperature\": [1.1, 0.8, 1.3]}");
        LayaConfig c = LayaConfig.fromRlAgentConfig(f);
        assertEquals(2048, c.maxLen());
        assertEquals(512, c.headMaxLen());
        assertArrayEquals(new double[]{1.1, 0.8, 1.3}, c.temperature());
    }
}