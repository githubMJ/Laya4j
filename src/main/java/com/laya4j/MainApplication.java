...
import com.laya4j.benchmark.Benchmark;
...
public class MainApplication {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 1. 从 HF Hub 拉取 tokenizer + config + ONNX 模型(1.2 GB FP32,满血版)
        ...
        System.out.printf("[init] 加载耗时 %d ms%n%n", System.currentTimeMillis() - t0);
    ...
    static void runTests(LayaPredictor p) throws Exception {
        ...
        for (int i = 0; i < alignInputs.length; i++) {
            ...
            System.out.printf("  %s java=%.4f py=%.4f diff=%.4f  %s%n",
                    ok ? "✅" : "⚠️ ", java, pyBaseline[i], diff, alignInputs[i]);
        }
        System.out.printf("  → 对齐 %d/%d%n", aligned, alignInputs.length);
    }

    static void printHelp() {
        System.out.println("Usage: MainApplication [options]");
        System.out.println("  --refresh-cache    强制从 HuggingFace 重新下载 tokenizer + config + ONNX");
    }
}
