...
public class LayaOnnxModel implements AutoCloseable {
    ...
    public LayaOnnxModel(Path onnxPath, Path tokenizerDir, String modelId, LayaConfig config) {
        if (onnxPath == null || !onnxPath.toFile().exists()) {
            throw new LayaException("ONNX model not found: " + onnxPath);
        }
        if (tokenizerDir == null || !tokenizerDir.toFile().isDirectory()) {
            throw new LayaException("Tokenizer dir not found: " + tokenizerDir);
        }
        ...
        try {
            ...
            throw new LayaException("Failed to load Laya model", e);
        }
    }
    ...
    public Map<String, Decision> predict(String state, List<Question> questions) {
        ...
        try {
            ...
            for (int i = 0; i < batch; i++) {
                ...
                idsBuf.put(Arrays_copy(tmp, maxSeqLen));
                for (int j = 0; j < bs.ids().length; j++) tmp[j] = 1;
                for (int j = bs.ids().length; j < maxSeqLen; j++) tmp[j] = 0;
                maskBuf.put(Arrays_copy(tmp, maxSeqLen));
                ...
                mposBuf.put(Arrays_copy(tmp, maxMarkers));
            ...
            throw new LayaException("ONNX inference failed", e);
        }
    }
    ...
    private static long[] Arrays_copy(long[] src, int len) {
        long[] r = new long[len];
