package com.laya4j.tetris.config;
...
import com.laya4j.tetris.laya.HeuristicLayaModel;
 ...
 * HuggingFace Hub 解析)。
 *
 * <p>切换方式:{@code -Dlaya.tetris.model=onnx}
...
public class ModelConfig {
    ...
    public LayaModel layaModel(@Value("${laya.tetris.model:mock}") String mode) throws Exception {
        if ("onnx".equalsIgnoreCase(mode)) {
            log.info("Loading real Laya ONNX model for Tetris decisions ...");
        ...
        log.info("Using heuristic mock LayaModel for Tetris (set laya.tetris.model=onnx for real weights)");
        return new HeuristicLayaModel();
    }
}
